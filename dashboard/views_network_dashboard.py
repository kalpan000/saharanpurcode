from dashboard.views import echart
...
def network_summary(request , returnjsonobj = True):    
    days = request.GET.get("days", 1)
    # if not days:
    #     days = 1
    # days = 1
    dc = DeviceCapibility.objects.all()
    type_list = ['Network','wlc']
    selectOptionDevices = []
    assetDevices = AddDevice.objects.filter(type_of_device__in = type_list)
    TOT_dev = 0
    for device in dc:
        ...
        for assetDevice in assetDevices:
            ...
            if (ipa == fullIP):
                ...
                break
    timeThreshold = datetime.now() - timedelta(hours=int(days))
    ...
    data["total_int"] = totalInterfaces
    data["total_dev"] = TOT_dev
