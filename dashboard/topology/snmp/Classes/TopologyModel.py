...
class NetworkTopology:
    ...
    def classifyDeviceHostnameToGroup(self, s_device_name, type,subtype,category):
    # def classifyDeviceHostnameToGroup(self, device):
        # PATTERNS COMPILATIOn
        # log("Hostname matched[key]: " + device)
        group = "2"  # for key (source hostname)
        image = "default.png"
        ...
        #             return ("4","L2.png")
        # if (type == "Server"):
        #     return ("1","server1.png")
        # if (type == "wlc"):
        #     return ("1","wlc.svg")
        # log("Final GROUP for key: " + type + " is " + group)
        # if device == "HWH-FS-02-01-02" or \
        #     device == "HWH-FS-02-01-01" or \
        #     device == "HWH-FS-03-01-01" or \
        #     device == "HWH-FS-04-01-01" or \
        #     device == "HWH-FS-05-01-01" or \
        #     device == "HWH-FS-06-01-01" or \
        #     device == "HWH-FS-07-01-01" or \
        #     device == "HWH-FS-08-01-01" or \
        #     device == "HWH-FS-09-01-01" or \
        #     device == "HWH-FS-10-01-01" or \
        #     device == "HWH-FS-00-05-03" or \
        #     device == "HWH-FS-17-01-01" or \
        #     device == "HWH-FS-19-01-01" or \
        #     device == "HWH-FS-11-01-01":
        # RAILTEL
        # if device.startswith("HWH-FS-"):
        #     return ("4","L2.png")
        # if device.startswith("wlc"):
        #     return ("3","wlc.svg")
        # if device.startswith("G-ER"):
        #     return ("1","ap.svg")
        # if device == "HWH-C920-WIFI2-T4-ER":
        #     return ("5", "L7.png")
        # RAILTEL

        # NIC

        if s_device_name == "CCC-R3R1-N9K-CORESW2":
            return ("5", "L2-switch.svg")
        ...
        if type == "Network":
            ...
            if subtype == "switch":
                if category == "AX":
                    ...
                    print("SWITCH AGGREGATION")
        if type == "Server":
            if s_device_name.endswith("LB1"):
                return ("1","lb.svg")
            return ("1","vm.jpeg")
        if type == "Blade Chassis":
            return ("1", "chasi.jpeg")
        if type == "Storage":
            return ("1", "storage.svg")
        return (group,image)

    def addDevice(self, s_device_name=None, type=None,subtype=None,category=None, device=None):
    # def addDevice(self, s_device_name):
        if not self.isDeviceDefined(s_device_name):
            group_number, image_name = self.classifyDeviceHostnameToGroup(s_device_name, type,subtype,category)
            # group_number, image_name = self.classifyDeviceHostnameToGroup(s_device_name)
            # self.visualization_dict['nodes'].append({"id": s_device_name, "group": group_number, "image": image_name , "protocol" : "snmp","ip":device, "type":type, "subtype":subtype,"category":category})
            self.visualization_dict['nodes'].append({"id": s_device_name, "group": group_number, "image": image_name , "protocol" : "snmp", "ip":device})
    ...
    def addInterfaceStats(self,deviceid,INDEX,ifDescr,ifType,ifOperStatus,ifHCInOctets,ifHCOutOctets,ifHighSpeed):
        
        if ifHCInOctets == "" or ifHCInOctets == " ":
            ifHCInOctets = 0
        if ifHCOutOctets == "" or ifHCOutOctets == " ":
            ifHCOutOctets = 0
        ...
        if deviceid not in self.interface_stats:
            ...
            # Here we have a found device and found non-empty interface stats
                ...
                if device_interface_stats['last_ifHCInOctets'] == "" or device_interface_stats['last_ifHCInOctets'] == " " : 
                    device_interface_stats['last_ifHCInOctets'] = 0
                if device_interface_stats['last_ifHCOutOctets'] == "" or device_interface_stats['last_ifHCOutOctets'] == " " : 
                    device_interface_stats['last_ifHCOutOctets'] = 0
                if device_interface_stats["last_timestamp"] == "" or device_interface_stats["last_timestamp"] == " " : 
                    device_interface_stats["last_timestamp"] = 0                
