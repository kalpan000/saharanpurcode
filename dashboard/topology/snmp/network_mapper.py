...
def main_with_args(args = False):
    ...
    if 'DEVICES' not in config or 'DEFAULT' not in config or 'TYPE' not in config:
    ...
    types = json.loads(config['TYPE']['types']) # Network, Server, VMS, LB, Storage, etc
    subtypes = json.loads(config['SUBTYPE']['types']) # Router, Switch
    categories = json.loads(config['CATEGORY']['types']) # AX, CORE, ToR, etx

    for i in range(0 , len(devices)):
        device = devices[i].replace('\n', '')
        communityString = communityStrings[i].replace('\n', '')
        type = types[i].replace('\n', '')
        subtype = subtypes[i].replace('\n', '')
        ...
        '''
        try:
            if device == "10.1.45.3" or device == "10.1.45.14":
                print("CORE DEVICE FOUND", device)
                sSNMPHostname = quicksnmp.get(device, sysName_named_oid,hlapi.UsmUserData('admin', authKey='Network@123', privKey='Network@123', privProtocol=hlapi.usmAesCfb128Protocol))
                print(' = '.join([x.prettyPrint() for x in sSNMPHostname]))
                # log(" This is how to get to numeric OID: " + str(tuple(sSNMPHostname[0][0])))
                # log(" This is how to get data only: " + str(sSNMPHostname[0][1]))
                print("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXGETTTTXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
            else:
        ...
        try:
            if device == "10.1.45.3" or device == "10.1.45.14":
                ...
                rawInterfacesTable = quicksnmp.get_table(device, interfaces_table_named_oid,hlapi.UsmUserData('admin', authKey='Network@123', privKey='Network@123', privProtocol=hlapi.usmAesCfb128Protocol))
                print(rawInterfacesTable)
                # for row in rawInterfacesTable:
                #     for item in row:
                #         log(' = '.join([x.prettyPrint() for x in item]))
                #     log('')
                print("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXINTERFACE TABLEXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
            else:
                # log("INTERFACES TABLE: ")
        ...
        try:
            if device == "10.1.45.3" or device == "10.1.45.14":
                # log("LLDP TABLE: ")
                # log("############")
                rawTable = quicksnmp.get_table(device, lldp_table_named_oid,hlapi.UsmUserData('admin', authKey='Network@123', privKey='Network@123', privProtocol=hlapi.usmAesCfb128Protocol))
                # for row in rawTable:
                #     for item in row:
                #         log(' = '.join([x.prettyPrint() for x in item]))
                #     log('')
        ...
        '''
        topology_model.addDevice(str(sSNMPHostname[0][1]), str(type), str(subtype), str(category), str(device))
        ...
        for row in rawTable:
            ...
            print("NEIGHBOURS INT VALUE", row[2][1], "\nXXXXXXXXXXXXXXXXXXXXXXYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYXXXXXXXXXXXXXXXXXXXXXx")
            # lldpRemSysName
            print('lldpRemSysName: ' + str(row[0][1]))
            # lldpRemSysDesc
            print('lldpRemSysDesc: ' + str(row[1][1]))
            # lldpRemPortId
            print('lldpRemPortId: ' + str(row[2][1]))
            # lldpRemPortDesc
            print('lldpRemPortDesc: ' + str(row[3][1]))

            print("_-------------------------------END------------------------------------________________")
            ...
            if device == "10.1.45.3" or device == "10.1.45.14":
                local_interface_name = quicksnmp.get(device, [('LLDP-MIB', 'lldpLocPortEntry', local_in_index)],hlapi.UsmUserData('admin', authKey='Network@123', privKey='Network@123', privProtocol=hlapi.usmAesCfb128Protocol))
            else:
                local_interface_name = quicksnmp.get(device, [('LLDP-MIB', 'lldpLocPortEntry', local_in_index)],
                                                 hlapi.CommunityData(communityString))
